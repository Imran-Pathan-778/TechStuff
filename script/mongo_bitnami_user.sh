...
create_user() {
    ...
    echo 
    mongo -u $ADMIN_USER -p $ADMIN_PASS --authenticationDatabase $DATABASE --authenticationMechanism SCRAM-SHA-256 $DATABASE <<EOF
...
list_users() {
    ...
    mongo -u $ADMIN_USER -p $ADMIN_PASS --authenticationDatabase $DATABASE --authenticationMechanism SCRAM-SHA-256 $DATABASE <<EOF | grep -o '"user" : "[^"]*"' | awk -F'"' '{print $4}'
    use $DATABASE_NAME
    db.getUsers()
    echo  
EOF
...
list_databases() {
    mongo -u $ADMIN_USER -p $ADMIN_PASS --authenticationDatabase $DATABASE --authenticationMechanism SCRAM-SHA-256 $DATABASE <<EOF | grep -o '"name" : "[^"]*"' | awk -F'"' '{print $4}'
